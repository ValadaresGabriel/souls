...
[CustomEditor(typeof(MoveBlock))]
public class MoveBlockEditor : Editor {
    MoveBlock m_MovingPlatform;
    ...
    {
        m_PreviewPosition = 0;
        m_MovingPlatform = target as MoveBlock;

        if(!EditorApplication.isPlayingOrWillChangePlaymode)
            MoveBlockPreview.CreateNewPreview(m_MovingPlatform);
    ...
    {
        MoveBlockPreview.DestroyPreview();
    }
    ...
    private void OnSceneGUI()
    {
        ...
        for (int i = 0; i < m_MovingPlatform.localNodes.Length; ++i)
        {
            ...
            {
                worldPos = m_MovingPlatform.inspector_worldNode[i];
            }
            else
            {
            ...
            if (i == 0)
            {
                if (m_MovingPlatform.platformType != MoveBlock.MovingPlatformType.LOOP)
                    continue;

                if (Application.isPlaying)
                {
                    Handles.DrawDottedLine(worldPos, m_MovingPlatform.inspector_worldNode[m_MovingPlatform.inspector_worldNode.Length - 1], 10);
                }
                else
                {
            ...
            else
            {
                if (Application.isPlaying)
                {
                    Handles.DrawDottedLine(worldPos, m_MovingPlatform.inspector_worldNode[i - 1], 10);
                }
                else
                {
    ...
    void MovePreview()
    {
        ...
        MoveBlockPreview.preview.transform.position = m_MovingPlatform.transform.TransformPoint(localPos);
